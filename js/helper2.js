define([
    ...
    function loadTable(response, fields, titlereporte, idtable, isexportable) {
                ...
                for (let j = 0; j < nfield; j++) {
                    ...
                    if(item==undefined){
                        ...
                        if(fields[j].fieldname=='OBJECTID'){
                            aux_tbody += `<td class='tdzoom' id="${item} "><span class='esri-icon-zoom-in-magnifying-glass' ></span></td>`;
                        }else{
        ...
        let table='';
        if(isexportable){
            table=getDataTableExport(columnstoexportar, titlereporte,lengthscrolly);
        }else{
    ...
    function getDataTableExport(columnstoexportar,titlereporte, lengthscrolly){
        
        // let titlereporte=titlereporte;
        console.log(titlereporte);
            ...
            buttons:  [
                {
                    ...
                    customize: function(doc){
                        ...
                        /**
                         * para aliniar celdas numero o cadena
                         */
                        //   for (i = 1; i < rowCount ; i++){
                        //     for(let j = 0; j < alignColum.length ; j++){
                        //         if (alignColum[j] == 'L'){
                        ...
                        let wcolumns = [];
                        for (let i = 0; i < columnstoexportar.length ; i++){
                            if (i==0) {
                                wcolumns.push("5%");
                            }
                            else {
                        ...
                        $(__idtable).find('tbody tr:first-child td').each(function(){
                        ...
                        // doc.content[0].table.widths = wcolumns;
                                ...
                                table: {
                                    ...
                                    body: [
                                        [
                                            ...
                                            {text: titlereporte, style:'header', fontSize: 13, bold:true, alignment:'center', border: [false,false,false,false]},
        ...
        // // utilizo un input personalizado para search
    ...
    function paintToZoom(objectid, url_servicio, _gl_search) {
        _gl_search.removeAll();
        ...
        _queryt.execute(_qparams).then(function(response){
            let geometrytype=response.geometryType,
                // symbol=gettypesymbol(response.features[0].geometry);
